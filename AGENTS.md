...
- Give each durable product fact one authoritative owner under `docs/documentation.md`. Behavior specifications own product state and action results; presentation specifications own exact reusable visual values; low-fidelity wireframes visualize linked contracts and do not independently define or repeat them. Update another document only when its own responsibility changes.
...
- Every new iteration contract records the full 40-character Git commit ID of its accepted product-contract baseline and links the applicable sources at that revision. Production implementation must be based on or otherwise include that baseline; later unrelated product changes do not silently expand the iteration.
...
- `main` is the single authoritative integration branch. A task branch or worktree isolates active execution; it is not a second source of product truth, a release status, or a durable development line.
...
- Prefer one short-lived branch for each currently active, independently integrable scope. Use lowercase scope-based names: `product/<short-scope>` for an accepted product-contract change that is not yet integrated, `work/iteration-<number>-<short-slug>` for iteration implementation, `work/task-<short-slug>` for other engineering or governance work, and `integration/<short-scope>` or `temp/<short-purpose>` only for bounded integration, recovery, or history-maintenance work. Do not create permanent `product`, `develop`, terminal-specific, contributor-specific, or version-status branches.
- A product-contract branch and implementation that depends on it are a dependency chain, not independent parallel lines. The author must accept the product boundary before production implementation proceeds, and the implementation branch must be based on or otherwise include that accepted contract. Product work may proceed concurrently with unrelated implementation only when their authoritative documents, interfaces, files, and acceptance boundaries have low overlap.
- Use a separate Git worktree for each genuinely concurrent active branch. One worktree carries one active scope; generated output, untracked files, and incomplete changes must not be shared or moved implicitly between worktrees. Machine-specific worktree paths are local coordination state and must not become authoritative project documentation.
...
- Integrate into the current `main` only after the branch is internally consistent and its applicable validation is complete. Re-check the integrated result when conflict resolution or cross-line interaction changes the evidence boundary. Rebase or otherwise rewrite only unpublished history and only within the applicable authorization; never rewrite shared history by default.
- Remote task branches exist only when cross-device continuation or actual collaboration requires them. Creating a local branch or worktree does not authorize publishing it.
- Remove a completed task branch or worktree only after confirming that it contains no unique commit, untracked file, generated evidence, or unresolved handoff. Cleanup remains a mutation within the applicable authorization boundary.
