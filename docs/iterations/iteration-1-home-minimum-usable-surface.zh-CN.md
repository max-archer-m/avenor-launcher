...
- Value: `Completed`
- Updated: 2026-08-11
- Basis: 作者已接受可观察的 Home 结果，实现和迭代文档均已 commit 到共享的 `origin/main` 历史。尚未完成的验证继续记录在下文中，不视为通过。
...
- 项目作者接受已交付实现和观察到的 Home 行为足以继续，且现有项目基础适合在不替换的情况下扩展后，可以开始迭代 2。已知验证缺口记录为建议后续事项，但不阻塞推进。该决定不豁免任何版本级退出门禁。
...
以下场景建议用于降低风险和完善证据。除非项目作者明确将某项提升为强制项，否则它们不是迭代 1 准出或迭代 2 准入门禁；适用的 `1.0.0` 完成门禁保持独立。
...
建议补充的证据；其中版本契约要求的事项仍须在正式关闭 `1.0.0` 前完成：
...
现有证据支持所记录的 `Completed` 状态。缺失建议证据不能支持相应检查已经通过或正式版本已经验收的声明。
...
项目作者已接受交付实现和观察到的 Home 显示，实现和文档也已 commit 并同步；迭代 1 为 `Completed`。剩余 CLI、自动化、lint、merged-manifest、模拟器、真机和聚焦行为场景继续作为证据缺口记录。任何缺失检查都不标记为通过，最终 `1.0.0` 门禁保持不变。
...
缺失的 CLI、自动化测试、release lint、依赖解析、merged-manifest、API 31 模拟器、Samsung API 36、Pixel API 37 和聚焦 Home 行为证据继续作为建议后续事项，并在 `1.0.0` 版本契约要求的范围内重新检查。它们默认不阻塞迭代 2 准入或迭代 1 准出。迭代 2 不得把这些移交证据表述为已完成，也不得以此隐藏后来发现的 Home 契约失败。迭代 2 实现仍需项目作者单独明确授权。
