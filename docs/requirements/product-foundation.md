...
- Edit-mode removal persists immediately and shows a non-blocking long-duration Snackbar with `Removed from favorites` and `UNDO`. Only the latest visible removal can be undone. Undo restores the exact identity, destination, position, list size, and any list or ribbon deleted because it became empty. Snackbar dismissal, a later removal, edit-mode exit, the Android system Home action, process recreation, or leaving Avenor discards that temporary snapshot without retaining undo history.
