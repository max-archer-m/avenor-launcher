...
- Home uses content-driven vertical sizing within `16dp` outer padding and `16dp` spacing between adjacent visible modules. Basic information wraps its required content and interaction targets. The primary-and-companion favorite composition grows up to the remaining safe height and then scrolls within the overflowing group; Home itself does not scroll. The conditional bottom secondary favorite region has zero height until its later user-created content and interaction are defined.
- The middle composition uses `8dp` internal padding and an `8dp` inter-group gap. Primary favorites use approximately the left 55% and companion favorites the right 45% of the remaining horizontal space. The ratio expresses product hierarchy rather than a user-adjustable split, and neither group borrows unused capacity from the other. Every item uses `8dp` internal padding on all sides. Primary favorites use 48dp icons, default 64dp-high interaction targets, and `16sp/24sp` names; companion favorites use 32dp icons, default 48dp-high targets, and `14sp/20sp` names. Both use normal weight, a `16dp` icon-to-name gap, system font scaling, and static one-line names with end ellipsis when needed.
- Drawer additions append to primary favorites by default without navigating to Home or entering edit mode. A full visible viewport does not reject an addition: each group stays stationary while its content fits and scrolls independently after overflow. Edit mode adds drag handles and shared editing surfaces around the visible Home modules; it supports in-group ordering, moving a favorite to a valid insertion position in the other group, and swapping group assignments and positions when the cross-group target is occupied. Back exits edit mode.
