...
- Drawer action-sheet additions append one application to the current physical-leftmost vertical favorite list by default without navigating to Home or entering edit mode; if none exists, the action creates one list at medium size. Reordering lists changes this default target. Home edit mode additionally provides centered `24dp` plus controls in at least `48dp` targets at each persisted list end, every existing secondary ribbon, one provisional vertical list while fewer than two lists exist, and one provisional ribbon while fewer than five ribbons exist. Each control opens Drawer multi-selection for its captured target; confirmation appends entries in selection order, and a provisional list or ribbon becomes persistent only with at least one confirmed entry. A full visible viewport does not reject an addition.
- Edit mode is entered through a favorite action sheet or a long-press on eligible basic-information blank space. It adds application drag handles, remove badges, add controls, ribbon-level fixed controls, and shared editing surfaces. A visible remove badge uses a `24dp` error-semantic circle with a `14–16dp` X over the icon's logical top-start edge; its non-conflicting target remains at least `48dp × 48dp`. Application and ribbon drag handles retain a visible `24dp` graphic and applicable full-height `48dp`-wide target. Same-container body crossings exchange and persist immediately; same-container gaps never insert. Cross-container occupied bodies and gaps show mutually exclusive exchange or insertion feedback and save once on valid release. Cross-container preview size remains stable until the target presentation applies after release. System Back and the Android system Home action exit edit mode; the Android system Home action also cancels any active Drawer multi-selection and resolves to normal Home.
- Application dragging uses `48dp` edge zones inside the current valid application viewport. After approximately `120ms` stabilization, only that container auto-scrolls on its own axis, with speed increasing toward the edge; leaving the zone, entering invalid space or fixed controls, reaching the boundary, cancellation, or interruption stops it. Complete-list and complete-ribbon drags never auto-scroll. An eligible edge fades in a non-interactive light-foreground gradient up to `8%` opacity below continuously recalculated insertion and exchange feedback; an unavailable direction shows no cue, and scrolling itself adds no haptic, arrow, text, ripple, or accessibility node.
- Edit-mode application removal persists immediately and shows a non-blocking long-duration Snackbar with `Removed from favorites` and `UNDO`. Only the latest visible removal can be undone. Undo restores the exact identity, destination, position, list size, and any list or ribbon deleted because it became empty. Snackbar dismissal, a later removal, edit-mode exit, the Android system Home action, process recreation, or leaving Avenor discards that temporary snapshot without retaining undo history.
- Each editable persisted ribbon has a fixed logical-start remove control, central horizontal application viewport, and fixed logical-end reorder rail. The rails are each `48dp × 56dp`, remain outside scrolling content, and do not resize `128dp` application items. One persisted ribbon hides reorder presentation, interaction, press feedback, and accessibility while preserving its end rail; at least two persisted ribbons show reorder on each. A provisional ribbon exposes neither ribbon-level action. Explicit complete-ribbon removal first uses a platform-default `Remove ribbon?` Dialog with explanatory body, `Cancel`, and destructive `Remove`; Back and outside tap cancel without mutation. Confirmation persists removal and offers latest-only `Ribbon removed` / `UNDO`. Automatic deletion after the last application leaves a ribbon does not show this Dialog.
- Complete-ribbon reorder uses a frozen, non-interactive static-layout preview of the entire ribbon window visible at drag start, including both rails, visible full or partial entries, boundary, and applicable edge fades, while preserving width, `56dp` height, horizontal scroll state, and grab offset. An equal source placeholder remains. The finger point entering another persisted ribbon body immediately exchanges and persists order with a short haptic step; gaps never insert, and invalid release or interruption retains the last persisted exchange.
...
- Given the user drags an application within one vertical list or one ribbon, when the touch point enters another application body, then Avenor exchanges and persists those positions immediately; same-container gaps do not insert, and later invalid release retains the last successfully persisted exchange.
- Given the user drags an application into another vertical list or ribbon, when the touch point enters an occupied application body, then Avenor shows only exchange feedback; valid release saves both identities' destinations and positions without loss or duplication.
- Given the user drags an application into another vertical list or ribbon, when the touch point enters a valid first, between-item, last, or provisional-empty boundary, then Avenor shows only an insertion line; valid release saves one insertion, preserves target relative order, applies target presentation, and deletes the source container if it becomes empty.
- Given edit mode contains one persisted ribbon, when ribbon controls are shown, then its end rail remains allocated but exposes no reorder graphic, interaction, press feedback, or accessibility node; given at least two persisted ribbons exist, every persisted ribbon exposes reorder, while a provisional ribbon exposes neither ribbon-level action.
- Given the user activates remove-ribbon, when the platform-default Dialog is open, then Home editing is blocked and Cancel, Back, or outside tap closes without mutation; when `Remove` is confirmed, then the complete ribbon is persisted as removed and latest-only `Ribbon removed` / `UNDO` can restore its position, membership, and application order.
- Given the user drags a ribbon's visible fixed reorder handle, when dragging begins, then Avenor freezes the complete currently visible ribbon window into a non-interactive preview with an equal source placeholder; when the finger enters another persisted ribbon body, Avenor highlights the target, exchanges and persists the two ribbon positions immediately, and produces a short haptic step, while ribbon gaps never insert.
...
- The only durable user-content data is each launchable application entry saved as a Home favorite together with its vertical-list or secondary-ribbon destination and user-defined position. Each vertical list has stable membership, order, and one large, medium, or small list-level presentation value; each ribbon has stable order within the secondary-favorites area and each secondary favorite has one position within its ribbon. Each stable entry identity has one favorite destination and must not be duplicated across lists or ribbons. Entry identifiers must distinguish primary, cloned, and work-profile entries exposed by the platform and must not store or deduplicate solely by package name. The latest edit-mode removal snapshot is transient Undo state rather than durable undo history.
