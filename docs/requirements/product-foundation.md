...
- Each editable ribbon has a fixed logical-start remove control, central horizontal application viewport, and fixed logical-end reorder handle. The rails are each `48dp × 56dp`, remain outside scrolling content, and do not resize `128dp` application items. Removing a ribbon persists the removal of its complete membership and offers latest-only `Ribbon removed` / `UNDO`; reordering ribbons exchanges and persists when the ribbon preview enters another ribbon body, without gap insertion.
- Every persisted vertical list has a fixed `48dp`-high edit-mode control bar inside the top of its existing viewport. Three equal physical slots reserve remove-list, future list-size, and reorder-list actions from left to right without RTL mirroring. The center size slot stays hidden and non-interactive; a provisional list has no bar, and one persisted list hides the reorder control while preserving its slot. Visible controls use `40dp` circular containers and `24dp` icons in separate `48dp` targets, with error semantics for removal and light-foreground semantics for reorder. They use opacity-only press feedback without ripple. The bar reduces only the edit-mode application viewport and never changes the favorite area's external height.
...
- Given Home edit mode shows a persisted vertical list, when its top control bar is presented, then remove appears in the physical-left slot, the future size slot remains allocated but invisible and non-interactive, and reorder appears in the physical-right slot only when two persisted lists exist; a provisional list has no bar and RTL does not mirror these positions.
...
- Given the user drags a ribbon's fixed reorder handle, when its complete preview enters another ribbon body, then Avenor exchanges and persists the two ribbon positions immediately; ribbon gaps never insert.
