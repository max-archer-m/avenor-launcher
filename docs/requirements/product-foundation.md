...
- Home uses content-driven vertical sizing within `16dp` outer padding and `16dp` spacing between adjacent visible modules. Basic information wraps its required content and interaction targets. The primary favorites area grows up to the remaining safe height and then scrolls within the overflowing group; Home itself does not scroll vertically. The secondary favorites area is below the primary favorites area, uses the full available Home content width, has zero height without content, and scrolls horizontally only when its content exceeds its visible width.
- Secondary favorites content is user-defined rather than divided by product-defined types. Individual items have no title and consist of launchable application entries matching the current favorite-entry model. Selecting an entry launches its application; long-pressing it opens the existing application action sheet. Each horizontal item list is finite with an explicit first and last entry and does not scroll cyclically.
...
- Given the secondary favorites area has content, when Home displays it, then it shows user-defined untitled launchable application entries without product-defined type grouping; selecting an entry launches its application, long-pressing it opens the existing application action sheet, and horizontal scrolling stops at the explicit first and last entries without wrapping.
