...
- Home 以安全可用高度的约 20:60:20 为构图目标：基础信息区、完整的主收藏与伴收藏构图，以及预留的底部次级功能区。未来次级能力尚未定义时，收藏构图也不得扩张进入底部区域。
- 中部构图内，主收藏约占左侧 60% 横向空间，伴收藏约占右侧 40%。该比例表达产品层级，而不是用户可调节的分栏比例，两组不得借用彼此的空余容量。主收藏使用 48dp 图标和至少 64dp 交互目标；伴收藏使用 32dp 图标和至少 48dp 交互目标。两组均展示静止的一行应用名称，必要时在末尾省略。
- 从 Drawer 新增时默认追加到主收藏，不跳转 Home，也不进入编辑模式。可见视口已经铺满时仍然接受新增：每组内容能够容纳时保持静止，溢出后各自独立滚动。编辑模式为收藏增加拖动柄，并为 Home 三个模块增加统一编辑表面；它支持组内排序、将收藏移动到另一组的空位置，以及在跨组目标已有收藏时交换双方的分组归属和位置。Back 退出编辑模式。
