...
- Define cross-group dragging as two mutually exclusive operations. An occupied target body performs cross-group exchange feedback: the target receives an exchange border, but neither group's visible list position or assignment changes during the drag. A target gap performs cross-group insertion feedback: an insertion line identifies the target boundary, but neither group's visible list changes before release. On release, apply the current cross-group exchange or insertion once; insertion removes the source favorite and preserves the target group's relative order. First, between-item, last, and empty-group boundaries are valid insertion targets.
...
- Do not persist the current favorite operation during dragging. In-group exchange updates visible positions as the touch point enters eligible bodies; cross-group exchange and insertion remain feedback-only and do not reorder either group. Finalize the current cross-group exchange or insertion once on release over the corresponding valid target. The stable source position is not a second interactive copy. An in-group exchange may animate its target, but cross-group feedback must not animate either list or move the independent preview.
...
5. Exchange across groups using the target group's first, middle, and last favorite, verifying that only exchange feedback appears during the drag, neither list changes before release, and the example-style group counts and source/target positions are correct after release.
...
8. Move from an exchange body to an insertion gap and back; verify cross-group feedback is mutually exclusive, neither list changes during the drag, and the current operation is applied and persisted only on release.
