...
The local candidate is configured with `versionName` `1.3.0` and `versionCode` `4`. Its accepted source is commit `bd535ea86bbc514916c3124db825c54cb1e1152f`; the locally built debug APK and signing identity are recorded under Iteration 21. This local candidate and evidence have not yet been integrated or synchronized to the designated shared Git history. A tag remains a separate author decision, not a promised result.
...
- Candidate source and evidence remain local until they are integrated and synchronized to the designated shared Git history.
...
Iterations 15-21 are `Completed`, and the author accepted the local `1.3.0(4)` candidate. The `Author daily-use baseline` version is not yet declared complete because the candidate source and this delivery evidence have not been integrated and synchronized to the designated shared Git history. No retained version artifact, tag, publication, or Release is claimed.
