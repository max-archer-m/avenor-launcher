...
The current candidate is configured with `versionName` `1.3.0` and provisional `versionCode` `4`. The source commit, signing category, artifact identity, completion decision, and any tag decision remain unperformed until their applicable later gates. The code becomes allocated only if a corresponding APK becomes traceable evidence under release governance. A tag is a possible post-acceptance author decision, not a promised result.
...
| [Iteration 21: Upgrade, Regression, and Version Closure](iteration-21-upgrade-regression-and-version-closure.md) | `In progress` | 2026-08-27 | Candidate-configuration work has started after acceptance and integration of Iterations 15-20. |
...
No version completion result exists. Iterations 15-20 are `Completed`, and Iteration 21 is
`In progress`, so `1.3.0` remains incomplete. No version
