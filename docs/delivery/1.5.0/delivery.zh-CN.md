...
| [迭代 28：升级、回归与版本收尾](iteration-28-upgrade-regression-and-version-closure.zh-CN.md) | `Completed` | 2026-09-07 | 作者在构建打包和主设备升级验收后接受修订后的样式设置面板布局，并指示本地完成、不生成 tag 或候选包；契约记录 2026-09-06 布局修订与 2026-09-07 1.5.0 范围边界。 |
...
[契约](iteration-28-upgrade-regression-and-version-closure.zh-CN.md)。2026-09-06 作者指示将修订后的共享样式设置面板布局在本迭代内交付与验证；契约已记录该修订。2026-09-07 作者确认 1.5.0 范围边界，将本地备份与恢复契约、隐藏名称与竖排列表几何呈现契约顺延至下一版本；契约已记录该修订。

作者于 2026-09-07 授权生产实现。实现将共享样式设置面板重构为标题行 + 内容行分块（标题行 32dp、内容行 48dp、应用尺寸内容行 56dp 并置于最后），采用 40dp 交互目标、紧凑 140dp x 40dp 双选项选择器与 28dp 步进控件，并在每个应用尺寸选项标签右侧增加 16dp 尾部间距。实现、1.5.0 标识更新（`versionName` `1.5.0`、`versionCode` 6）与本交付记录在同一交付提交中。

2026-09-07 作者报告证据：样式设置面板改动可打包且基本验收通过；1.5.0 标识已就位；从已接受前版本在主设备上直接升级基本通过。作者指示本地完成，不生成 tag 或正式候选 APK 产物；"可追溯候选包"要求因此记录为作者指示的范围外项，版本完成与 tag 处置仍由作者单独决策。对迭代 26–27 的源码层面契约核对在所选范围内未发现未解决的重大不一致。未执行检查，包括更广设备/API/OEM 矩阵及超出所报告升级验收的旅程回归，仍如实记录为缺口，而非通过。
...
迭代 26–28 均按上述实现和作者报告验收结果标记为 `Completed`。作者接受了修订后的样式设置面板布局与 1.5.0 标识更新，报告主设备上基本升级验收通过，并指示本地完成、不生成 tag 或候选包。本记录不授权推送、产物移动、发布或发行；这些操作仍需单独授权。
