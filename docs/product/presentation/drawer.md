...
- Drawer presents four setting blocks through the shared panel: application arrangement, section anchors, Drawer background, and application size. The application-size block is last because its content line is the tallest. The section-anchor and Right-or-Below controls use the shared compact two-option selector; application size and items per row use the shared application-size selector and stepper. Drawer retains its own setting names, option labels, availability, and save behavior.
- The Drawer-background block presents `Transparent` and `Frosted glass` in one `224dp x 40dp` two-option single-selection frame with `2dp` internal padding and two equal `110dp x 36dp` visible thumbs. Simplified Chinese uses `透明` and `磨砂玻璃`. The complete localized labels remain visible without wrapping or abbreviation. Each option retains a distinct interaction target at least `40dp` high through the owning content line; the visible thumb does not reduce that target.
