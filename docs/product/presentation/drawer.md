...
- Multi-selection rows reuse the ordinary inventory's items-per-row grid and arrangements; row geometry matches ordinary Content, and the selection state changes no layout. Multi-selection rows show no press Ripple; the selection scale and outline treatment is their visible response instead.
- An available unselected row shows no selection indicator; the multi-selection mode is expressed by the top app bar.
- A selected row scales its complete cell to approximately `90%` as a purely visual transform with unchanged layout bounds, restoring `100%` on deselection; the scale animation uses the shared short-duration property-animation token. It also draws one complete `2dp` `primaryTextColor` selection outline with `12dp` corners, and carries one overlaid order badge at its top-left corner.
- The order badge overlays the cell without occupying layout space: `primaryTextColor` background, one-based plain-text order in medium-weight shared `secondaryTextFontSize` and line height using `darkSurfaceBaseColor`, `12dp` corners at the top-left and bottom-right only with the other two corners square, and a minimum size of approximately `20dp` adapting to its content with `4dp` horizontal content padding. The badge is not an independent interaction target.
- Disabled already-favorited rows receive no badge, outline, or scaling and use the shared disabled semantic opacity for icon and name.
