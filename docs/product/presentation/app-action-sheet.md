...
- The sheet container has `0dp` global content padding. Each internal region owns its spacing. Content beyond the available height scrolls only in the application-shortcut region; the identity row, informational lines, and applicable Launcher-action row remain visible.
...
- Each informational line is one non-interactive full-width row at least `48dp` high with `16dp` horizontal content insets. Its text uses normal-weight shared `secondaryTextFontSize` with its line height in `secondaryTextColor`, remains static on one line, and uses end ellipsis when necessary.
