...
- The favorite composition has `8dp` internal padding on each side and a `16dp` gap between its 55:45 groups. Each item uses `8dp` internal padding on all sides and a `16dp` icon-to-name gap. Normal and edit-mode item heights are identical; drag handles do not stretch rows.
...
- In Home edit mode, the dragged item preview follows the pointer across the Home safe content area at approximately `50%` opacity. Only the two favorite groups accept a drop; insertion lines and swap borders are pointer-target feedback and commit on release.
