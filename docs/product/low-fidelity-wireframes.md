...
- Drawer additions enter primary favorites by default. In Home edit mode, the source position remains stable while an independent preview follows the touch point with its fixed grab offset. The preview does not participate in list animation. Exchange targets may update visible positions during the drag, while saved order and group assignment are finalized on release; insertion changes remain deferred until release.
...
- Only the two favorite groups accept a drop. Target feedback and edge auto-scroll use the finger touch point. Exchange targets update during the drag; cross-group insertion remains feedback-only until release. The current operation is finalized and saved on release.
