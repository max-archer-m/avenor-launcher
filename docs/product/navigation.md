...
- Process restoration begins at Home. Drawer, Settings, an action sheet, edit mode, and their transient positions are not restored after process recreation. Within the same process, returning through Back or another ordinary Avenor surface round trip preserves each vertical favorite list's meaningful vertical position and each favorite bar's meaningful horizontal position. Exiting edit mode preserves those positions after editing controls are removed. Process recreation resets every vertical list and favorite bar to its content start.

## Android system Home action and external application return

- When Avenor is the selected default Launcher, the Android system Home action from any Avenor surface, including Home edit mode, Drawer, Drawer favorite multi-selection mode, and Settings, displays normal Avenor Home. An active favorite multi-selection is cancelled and edit mode is exited. Within the same process, normal Home retains the latest meaningful vertical-list and favorite-bar positions formed by successfully completed scrolling or editing; cancelling an unfinished gesture does not create a new position. This is a system navigation result, not an Avenor Back or Drawer-to-Home transition, and Avenor does not reinterpret the Android system Home action as Back. Unqualified `Home` in this contract means the Avenor product surface, never this platform action.
...
- The same-process return preserves the Home favorite state, every vertical favorite list's meaningful vertical position, and every favorite bar's meaningful horizontal position according to the existing restoration rule. It does not restore Drawer, Settings, an action sheet, edit mode, or an in-progress Drawer position.
