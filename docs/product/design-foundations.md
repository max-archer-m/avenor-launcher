...
- Secondary-favorite entries use the medium dimensions: a `40dp` icon, `56dp` item height, `16dp` icon-to-name gap, and one static end-ellipsized `16sp/24sp` normal-weight name line. The icon is the primary recognition cue and the name is a secondary visual label. Items have a fixed width of `128dp`, remain left-aligned with `8dp` between items, and do not redistribute unused ribbon width. Adjacent ribbons use `8dp` vertical spacing. Each item uses the light foreground semantic color at `6%` opacity as its background, the same semantic color at `12%` opacity for a `1dp` boundary, and a `12dp` corner radius; the current light ripple remains its press feedback.
...
- A persisted vertical favorite list uses a fixed `48dp`-high edit-mode control bar with three equal slots at physical left, center, and right. The current remove-list and reorder-list controls use a `40dp` circular visible container and `24dp` icon centered in separate `48dp × 48dp` targets; the future size slot remains allocated but hidden and non-interactive. The remove control uses error semantics, while reorder uses the light-foreground role. These list-level circular controls are an explicit exception to the global ripple rule: their only press indication is a short increase in container opacity, retained throughout an active reorder drag. Physical left-to-right placement does not mirror for RTL locales.
- An editable secondary ribbon uses fixed `48dp × 56dp` control rails outside its central application viewport: remove-ribbon at logical start and ribbon reorder at logical end. Their visible `24dp` X and drag-handle graphics remain distinct from application-level controls, and subtle `1dp` dividers separate the fixed rails from scrolling content. The rails reduce only the edit-mode viewport; they do not resize `128dp` application items.
