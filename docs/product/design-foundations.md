...
- Home uses content-driven vertical sizing. The basic-information module wraps its required content and interaction targets; the primary favorites area grows up to the remaining safe height and handles overflow inside its groups. The secondary favorites area sits below it with `16dp` vertical spacing, uses the full available Home content width, has zero height without content, and scrolls horizontally only when its content exceeds its visible width. Home itself does not scroll vertically.
