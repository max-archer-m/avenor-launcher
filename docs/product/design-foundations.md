...
- Home uses content-driven vertical sizing and does not scroll vertically as a whole. The basic-information module wraps its required content and interaction targets. When the natural primary and populated-secondary heights plus their `16dp` separation fit the remaining safe height, both remain at natural height and primary favorite groups do not scroll. On overflow, the secondary area retains its natural ribbon height and the primary area receives the remaining viewport above it; only overflowing primary or companion groups scroll vertically. The full-width secondary area has zero height without content and contains at most five `56dp`-high horizontal ribbons, each independently scrolling horizontally only when its fixed-width entries overflow.
...
- Primary favorites carry greater visual prominence through `40dp` application icons, `8dp` item padding on every side, default `56dp`-high interaction targets, and `16sp/24sp` normal-weight names. Companion favorites use `32dp` application icons, the same item padding, default `48dp`-high interaction targets, and `14sp/20sp` normal-weight names. Both use a `16dp` icon-to-name gap, follow system font scaling, remain directly accessible, and display static one-line names with end ellipsis when needed. Normal mode and edit mode preserve these item heights; both groups use a `24dp` drag-handle graphic inside a `48dp`-wide hit region (`24dp` graphic plus `12dp` horizontal padding on each side). The hit-region height follows the current item height. The graphic has no additional right margin; its visible spacing from the application content comes from the item's existing padding.
- In Home edit mode, each currently defined editable module receives the same subtle translucent light-gray editing surface with a small corner radius. The current basic-information and primary-favorites-area modules receive surfaces; the secondary favorites area's editing surface is not defined by this contract. The surface sits behind content rather than tinting it and does not imply that a non-favorite module is editable. Exact shared color, opacity, and radius tokens remain subject to visual calibration.

- Secondary-favorite entries match primary-favorite dimensions: a `40dp` icon, `56dp` item height, `16dp` icon-to-name gap, and one static end-ellipsized `16sp/24sp` normal-weight name line. The icon is the primary recognition cue and the name is a secondary visual label. Items have a fixed width of `128dp`, remain left-aligned with `8dp` between items, and do not redistribute unused ribbon width. Adjacent ribbons use `8dp` vertical spacing. Each item uses the light foreground semantic color at `6%` opacity as its background, the same semantic color at `12%` opacity for a `1dp` boundary, and a `12dp` corner radius; the current light ripple remains its press feedback.
