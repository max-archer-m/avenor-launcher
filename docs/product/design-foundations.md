...
- In Home edit mode, each currently defined editable module receives the same subtle translucent light-gray editing surface with a small corner radius. The current basic-information and favorite-list-area modules receive surfaces; the secondary favorites area's editing surface is not defined by this contract. The surface sits behind content rather than tinting it and does not imply that a non-favorite module is editable. Exact shared color, opacity, and radius tokens remain subject to visual calibration.
