...
- Home targets an approximate 20:60:20 division of safe available height for basic information, the complete primary-and-companion favorite composition, and the reserved bottom secondary region. The favorite container does not expand into an empty bottom region.
- Home's middle favorite composition allocates approximately the left 60% of its horizontal space to primary favorites and the right 40% to companion favorites. The two areas form one asymmetric composition, keep independent scroll state, do not borrow space from one another, and the ratio is not a user-adjustable control.
- Primary favorites carry greater visual prominence through `48dp` application icons and at least `64dp` interaction targets. Companion favorites use `32dp` application icons and at least `48dp` interaction targets. Both remain directly accessible, visually legible, and labeled.
- In Home edit mode, the basic-information, favorite-composition, and reserved-bottom regions each receive the same subtle translucent light-gray editing surface with a small corner radius. The surface sits behind content rather than tinting it and does not imply that the non-favorite regions are editable. Exact shared color, opacity, and radius tokens remain subject to visual calibration.
