...
- Home's middle favorite composition uses `8dp` internal padding and an `8dp` inter-group gap, then allocates approximately the left 55% of the remaining horizontal space to primary favorites and the right 45% to companion favorites. The two areas form one asymmetric composition, keep independent scroll state, do not borrow space from one another, and the ratio is not a user-adjustable control.
- Primary favorites carry greater visual prominence through `48dp` application icons, `8dp` item padding on every side, default `64dp`-high interaction targets, and `16sp/24sp` normal-weight names. Companion favorites use `32dp` application icons, the same item padding, default `48dp`-high interaction targets, and `14sp/20sp` normal-weight names. Both use a `16dp` icon-to-name gap, follow system font scaling, remain directly accessible, and display static one-line names with end ellipsis when needed.
