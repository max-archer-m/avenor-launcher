...
Avenor stores your selected Drawer application size, name placement, items-per-row count, section-anchor presentation, and background mode with its blur-intensity level locally on the device. These display settings are used only to restore the presentation you selected and are not application-usage history.
