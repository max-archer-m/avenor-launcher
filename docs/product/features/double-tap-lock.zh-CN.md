...
> 英文语义源：[double-tap-lock.md](double-tap-lock.md)。
...
- 每次以启用服务为目的跳转系统无障碍设置前，Avenor 单独展示显著披露，并提供 `取消` 与 `继续`。继续只确认本次跳转并打开系统界面；Avenor 不需要保留披露确认历史，继续也不表示 Android 已经启用服务。
...
本地隐私说明必须说明无障碍服务、唯一的锁屏用途、是否读取或收集数据、用户如何启用或关闭，以及授权由 Android 控制。
