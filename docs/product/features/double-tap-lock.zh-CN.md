...
锁屏是可选的 Home 能力。用户在快捷操作中绑定为 `锁屏` 的 Home 槽位，通过 Avenor 用途受限的无障碍服务请求 Android 系统锁屏操作。

该能力在用户同时完成两件事之前保持无效：在 [quick-actions.zh-CN.md](quick-actions.zh-CN.md) 中把某个快捷操作槽位绑定为 `锁屏`，并在系统设置中明确启用 Avenor 无障碍服务。它不得阻断 Home、Drawer、应用启动、Settings 或其他核心路径。Avenor 不在首次启动时请求该能力。
...
- 该行在快捷操作设置页中始终存在；所需 Avenor 无障碍服务已经启用并连接时副标题显示 `已开启`，其他情况显示 `已关闭`。它是本能力的常设状态与授权通道，与当前槽位绑定无关。
...
- 每次以启用服务为目的跳转系统无障碍设置前，Avenor 单独展示 [privacy.zh-CN.md](privacy.zh-CN.md#锁屏显著披露) 定义的显著披露，并提供 `取消` 与 `同意并继续`。同意并继续只确认本次跳转并打开系统界面；Avenor 不保留披露确认历史，该操作也不表示 Android 已经启用服务。
...
- Given 某槽位绑定为 `锁屏` 且服务已经启用，when 绑定手势在符合条件的空白区域被识别且没有其他手势取得所有权，then Avenor 请求一次系统锁屏操作。
...
- Given 服务已经启用但操作失败，when 绑定手势被识别，then Avenor 保持在 Home、展示一次本地化失败提示，且不重试。
