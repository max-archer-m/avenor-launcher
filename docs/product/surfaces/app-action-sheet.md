...
- Selecting Edit closes the sheet and enters collapsed-panel application-level Home edit mode at the current main-list and ribbon positions. It does not select a module, scroll, reveal another module, or expand the style panel. The same action requests one non-blocking inventory refresh without delaying entry.
