...
Home uses content-driven vertical sizing rather than fixed proportional regions and never scrolls vertically as a whole. The basic-information region wraps its required content and interaction targets. When the natural heights of the primary favorites area and a populated secondary favorites area, including their `16dp` separation, fit within the remaining safe height, both use their natural heights and neither primary favorite group scrolls vertically. When their combined natural height exceeds that boundary, the secondary favorites area retains its natural ribbon height and the primary favorites area uses the remaining viewport above it; only overflowing primary and companion groups scroll vertically inside that viewport. The secondary favorites area uses the full available Home content width and has zero height when it has no content. Each ribbon handles its own horizontal overflow without moving other Home regions. Any remaining screen space is unallocated transparent Home space, not a persistent product region.

Within the middle favorite composition, the primary-favorite area uses approximately the left 55% and the companion-favorite area the right 45% of the available horizontal space. The composition uses `8dp` internal padding on each side and a `16dp` gap between the two groups; the 55:45 split applies to the remaining width after those fixed spaces. This is a compositional hierarchy, not a user-adjustable divider. “Calm” and “restrained” do not impose a deliberately low favorite count; they constrain distraction and avoidable operations. Both groups share one favorite-composition viewport height, which grows to the larger content requirement of the two groups up to the safe height remaining after the basic-information region and any populated secondary favorites area, and independently scroll their content only when it exceeds that shared viewport.
...
- Every favorite belongs to exactly one direct-access group: primary, companion, or secondary favorites. A favorite added through the currently defined Drawer action is assigned to the primary group by default. The current Home edit interaction moves favorites between the primary and companion groups; assignment to or from the secondary group is not yet defined. Adding a favorite does not navigate to Home or enter edit mode.
...
- The same stable launchable-entry identity cannot be stored or displayed more than once within a group or across the three favorite groups.
- Primary, cloned, and work-profile launchable entries remain distinct identities. They may be assigned independently, including to different favorite groups, and must not be deduplicated solely by package name.
...
- Each secondary-favorite entry displays a `40dp` platform application icon and one static application-name line within a `128dp`-wide item. Its height, icon size, `16dp` icon-to-name gap, and `16sp/24sp` normal-weight name match current primary-favorite presentation. The icon is the primary recognition cue and the name is a secondary visual label. A name that does not fit uses end ellipsis.
...
- Secondary favorites follow the same unique-assignment rule as primary and companion favorites. Assigning an identity to the secondary group changes that identity's single group assignment rather than creating a second favorite copy.
- The current contract does not yet define the entry for adding to the secondary group, the treatment offered when an identity is already favorited, or the interaction for moving an identity between the secondary group and the primary or companion group.
...
- Each ribbon has an explicit first and last entry. The current contract does not yet define ribbon creation, deletion, naming, membership editing, or ordering.
...
- The secondary favorites area appears below the primary favorites area only when it has content, uses the full available Home content width, and contains at most five `56dp`-high horizontal ribbons of user-defined launchable application entries. Each `128dp`-wide entry shows a `40dp` icon and one static application-name line, ribbons scroll horizontally only when their content overflows, and reaching a ribbon's last entry does not wrap to its first. Selecting an entry launches its application, and long-pressing it opens the existing application action sheet.
- Every saved launchable-entry identity appears at most once across primary, companion, and secondary favorites. Primary, cloned, and work-profile entries remain independently assignable identities even when they share a package family.
