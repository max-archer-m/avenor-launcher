...
- Edit mode preserves the normal-mode favorite item heights: primary items remain `56dp` high and companion items remain `48dp` high. Drag handles are contained within those rows and must not increase row height or stretch, compress, or remeasure either group's list.
- Dragging within the same group reorders that group. Crossing into the other group targets that group's valid insertion positions or occupied entries: dropping at an insertion position moves the dragged favorite into the target group, while dropping on an occupied entry swaps the two favorites' group assignments and positions. A cross-group operation never removes, overwrites, or unfavorites either entry.
- During a valid cross-group drag, the dragged favorite becomes a `50%`-opacity preview that follows the pointer across the complete Home safe content area, including both favorite groups and the space between them. The preview does not change the visible row height, viewport height, or list stretching. Insertion lines and swap borders are determined only by the pointer location: an insertion line appears between entries, while an occupied target receives a swap highlight. Other Home areas allow the preview to pass through but do not accept a drop. Dragging near the leading or trailing edge of either overflowing group automatically scrolls that target group so off-screen stored positions remain reachable.
- No cross-group mutation is committed during the drag. On release over a valid insertion or swap target, the move or swap is applied once, the resulting entries adopt their group specifications, and the change is saved immediately. An invalid drop returns the favorite to its original position without changing saved state. The completed swap may use a short synchronized position-confirmation animation; animation does not change row height or display intermediate swaps. If persistence fails, restore the last successfully saved group assignments and order, do not present the operation as completed, and show the existing favorite-update failure feedback.
...
- Releasing an in-group item keeps and immediately saves its current position. Cross-group release follows the insertion, swap, preview, invalid-drop, and save rules above.
