...
Within the middle favorite composition, the primary-favorite area uses approximately the left 55% and the companion-favorite area the right 45% of the available horizontal space. The composition first subtracts its fixed `8dp` left padding, `8dp` right padding, and `8dp` inter-group gap; the 55:45 split applies to the remaining width. Layout rounding may produce only the minimum unavoidable pixel error and must not create an additional visible gap or overlap. This is a compositional hierarchy, not a user-adjustable divider. “Calm” and “restrained” do not impose a deliberately low favorite count; they constrain distraction and avoidable operations. The two groups share one favorite-composition viewport height while keeping independent vertical scroll positions. The viewport grows to the larger content requirement of the two groups, up to the remaining safe available height; the group with less content preserves unused space rather than stretching its items or lending space to the other group.
...
- Dragging within the same group reorders that group. Crossing into the other group targets that group's valid insertion positions: dropping at a valid insertion position moves the dragged favorite into the target group, while dropping onto an occupied position swaps the two favorites' group assignments and positions. There are no predefined empty slots or product-defined fixed favorite capacity. A cross-group operation never removes, overwrites, or unfavorites either entry.
...
- Empty and populated favorite states preserve the same fixed Home structure. Primary and companion favorites remain part of one middle-screen composition. After subtracting the composition padding and inter-group gap, the remaining width approximately follows the 55:45 horizontal hierarchy. The groups share the composition viewport height, while a group stays stationary when its content fits and scrolls independently when its content overflows.
