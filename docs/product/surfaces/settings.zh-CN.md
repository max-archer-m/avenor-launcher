...
该标题只用于组织行为契约，不是 Settings 中可见的分组标题；下面两个入口以主要设置项的形式纵向叠加展示。

- **备份收藏与设置：** 点击后打开系统文档选择器选择本地保存位置，并写入一个 JSON 文件，其中包含当前完整的 Home 收藏状态（有序收藏模块的类型、顺序、稳定身份、样式和模块内应用顺序）以及完整的 Drawer 显示设置（应用尺寸、名称位置、每行数量、分组锚点表现和背景）。文件带 schema 版本号且不加密；不需要新增权限。导出文件由用户管理，位于 Avenor 应用私有存储之外。写入失败时显示本地化短提示 `无法备份收藏与设置`。
- **从备份恢复：** 点击后打开系统文档选择器选择备份文件，然后显示确认弹窗，说明确认后将替换当前的收藏与显示设置，并提供 `恢复` 与 `取消` 操作；`取消` 不产生任何变化。确认后以一次原子操作把完整当前状态替换为备份状态。恢复不对备份执行当前应用清单校验；随后由普通清单刷新规则处理——禁用身份保留、确认消失的身份移除、启动失败显示普通启动失败反馈。不可读、格式错误或版本不兼容的文件在不覆盖当前状态的前提下失败，并显示本地化短提示 `无法从备份恢复`。不含任何模块的空备份有效。恢复不触发一次性 Home 模型采用重置；Avenor 不做任何自动备份或上传该文件。
