...
| Favorite | 收藏应用 | 只保存到一个 Home 收藏目标的可启动应用条目；同一稳定条目身份不得跨列表或收藏栏重复 |
| Favorite-list area | 收藏列表区 | 由最多两个地位相同纵向收藏列表组成、按物理位置从左到右排列的 Home 模块；一个列表使用全宽，两个列表平均分配可用宽度 |
| Favorite size | 收藏尺寸 | 统一作用于某个纵向收藏列表全部条目的持久化“大／中／小”列表级呈现；每个列表初始使用“中”，并可在编辑模式中独立调整 |
| Secondary favorites area | 次级收藏区 | 收藏列表区下方、最多包含五条无标题且固定“中”尺寸用户收藏条目横向收藏栏的条件性 Home 模块 |
| Favorite container | 收藏容器 | 某个纵向收藏列表或某条收藏栏；同容器应用拖动即时交换，跨容器拖动在松手时交换或插入 |
| Vertical-list controls | 纵向列表控件 | 持久化纵向列表的固定编辑模式顶部栏：物理左侧删除、中间选择当前尺寸，以及存在两个列表时物理右侧的排序操作 |
| Favorite bar controls | 收藏栏控件 | 位于持久化收藏栏滚动应用视口之外的固定编辑模式控制轨：逻辑 start 确认删除整条收藏栏，至少存在两条持久化收藏栏时逻辑 end 调整收藏栏顺序 |
