...
| Secondary favorites area | 次级收藏区 | 收藏列表区下方、最多包含五条无标题且固定“中”尺寸用户收藏条目横向织带的条件性 Home 模块 |
...
| Ribbon controls | 织带控件 | 位于持久化织带滚动应用视口之外的固定编辑模式控制轨：逻辑 start 确认删除整条织带，至少存在两条持久化织带时逻辑 end 调整织带顺序 |
