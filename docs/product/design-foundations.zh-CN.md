...
- 共享文字光晕是渲染在适用前景文字背后的一层不可交互的胶囊形背景：中心为 `8%` 不透明度的黑色（`#14000000`），向胶囊边界逐渐衰减至完全透明，两端为大圆角的胶囊轮廓。它替换、绝不叠加该文字上的任何逐字形阴影。它不占据布局空间，不是交互目标，也不改变文字颜色。每份表现规范把共享文字光晕指派给其适用的文字。实现技术、衰减曲线、胶囊高度和文字两侧外扩范围为 `To be decided`，需在作者设备上校准。
