...
- 产品决策与技术架构决策分开记录。
- 不在产品文档中规定实现细节。
- 按照 `docs/documentation.md` 为每项持久产品事实指定一个权威归属。行为规范负责产品状态和操作结果；表现规范负责精确可复用视觉数值；低保真图可视化所链接契约，不独立定义或重复这些规则。只有另一份文档自身职责发生变化时，才更新该文档。
...
- 每份新迭代契约记录其已接受产品契约基线的完整 40 位 Git commit ID，并链接该修订下适用的来源。生产实现必须基于或以其他方式包含该基线；后续不相关的产品变化不得静默扩大迭代范围。
