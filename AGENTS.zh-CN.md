...
- 每个当前活跃且可独立集成的任务或迭代，宜使用一条短期分支。不得创建以终端命名的长期分支，也不得在终端专属分支中提前累积多个未来迭代。分支名应基于工作范围并使用小写形式：迭代工作使用 `work/iteration-<number>-<short-slug>`，其他任务使用 `work/task-<short-slug>`。
